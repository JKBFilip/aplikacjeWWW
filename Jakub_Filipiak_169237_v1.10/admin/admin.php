...
case 'delete_product':
    ...
    break;
		...
		echo '<a href="admin.php?action=list_products" style="padding:10px; background-color:#007BFF; color:white;">Zarządzaj Produktami</a><br>';
        echo ListaPodstron($link);
    }
} else {
    // --- Obsługa logowania ---
...
function ListaProduktow($link) {
    $query = "SELECT p.*, k.nazwa AS kategoria_nazwa 
              FROM produkty p 
              LEFT JOIN zarzadzanie k ON p.kategoria_id = k.ID 
              ORDER BY p.ID DESC";
    $result = mysqli_query($link, $query);
